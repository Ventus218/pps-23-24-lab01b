...
public class GUI extends JFrame {
    ...
    public GUI(int size, int numberOfMines) {
        ...
        ActionListener onClick = (e) -> {
            ...
            boolean aMineWasFound = logics.hit(pos); // call the logic here to tell it that cell at 'pos' has been
                                                     // seleced
            if (aMineWasFound) {
                quitGame();
    ...
    private void drawBoard() {
        for (var entry : this.buttons.entrySet()) {
            ...
            var position = entry.getValue();
            String text = null;
            var cell = logics.getCell(position);
            if (cell.wasHit()) {
                button.setEnabled(false);
                if (cell.hasMine()) {
                    text = "*";
                } else {
                    text = String.valueOf(logics.numberOfAdjacentMines(position));
                }
            } else {
